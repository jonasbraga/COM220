...
class LimConsultaEstudantes():
  def __init__(self, str, tipo):
    if tipo: 
      messagebox.showinfo('Aluno encontrado', str)
    else:
...
class CtrlEstudante():       
  def __init__(self):
    self.listaEstudantes = [
      Estudante('1001', 'Jonas Braga'),
      Estudante('1002', 'Pedro Soares'),
      Estudante('1003', 'Maria Eduarda'),
      Estudante('1004', 'Karen Iura')
    ]
  ...
  def consultaEstudantes(self):
    self.limiteConsulta = LimiteConsultaEstudantes(self)
  ...
  def consultaHandler(self, event):
    nroMatric = self.limiteConsulta.inputNro.get()
    self.limiteConsulta.inputNro.delete(0, len(self.limiteConsulta.inputNro.get()))
    for matric in self.listaEstudantes:
      if matric.getNroMatric() == nroMatric:
        strr = str(matric.getNroMatric() + ' ' + matric.getNome()+'\n')
        self.consulta = LimConsultaEstudantes(strr, True)
        return
    self.consulta = LimConsultaEstudantes('Aluno não encontrado', False)
